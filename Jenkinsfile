node {
    ...
    def application_name= "app_converter-shape"
    

        stage('Checkout') {
            git branch: 'main',
            url: 'https://github.com/spatialive/converter-shape.git'
        }
        stage('Validate') {
        ...
        def scannerHome = tool 'sonarqube-scanner';
                    ...
                    -Dsonar.projectKey=converter-shape \
                    -Dsonar.sources=. \
                    -Dsonar.css.node=. \
    ...
    stage('Push Image to Registry') {
        ...
        stage('Deploy container on DEV') {
                        ...
                        configFileProvider([configFile(fileId: "$File_Json_Id_APP_LAPIG_CONVERTER_SHAPE_HOMOL", targetLocation: 'container-converter-shape-deploy-homol.json')]) {
                            ...
                            response = sh(script: "curl -v -X POST -H 'Content-Type: application/json' -d @container-converter-shape-deploy-homol.json -s $url", returnStdout: true).trim()
                            echo response
                        }
